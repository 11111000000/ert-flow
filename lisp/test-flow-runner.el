    ...
    (pcase runner
      ...
      (_
       (let* ((ext (if (fboundp 'test-flow--conf)
                     ...
                     nil))
              ;; Lightweight auto-detect when called before panel open/detect-runner
              (ext (or ext
                       (when (stringp root)
                         (let* ((cand1 (expand-file-name "tests/run-tests.el" root))
                                (cand2 (expand-file-name "t/run-tests.el" root))
                                (cand3 (expand-file-name "test/run-tests.el" root)))
                           (cond
                            ((file-exists-p cand1)
                             ...
                             (list "emacs" "-Q" "--batch" "-l" cand1))
                            ((file-exists-p cand2)
                             (when (fboundp 'test-flow--set-conf)
                               (test-flow--set-conf sess 'external-command (list "emacs" "-Q" "--batch" "-l" cand2))
                               (test-flow--set-conf sess 'runner 'external-command))
                             (list "emacs" "-Q" "--batch" "-l" cand2))
                            ((file-exists-p cand3)
                             (when (fboundp 'test-flow--set-conf)
                               (test-flow--set-conf sess 'external-command (list "emacs" "-Q" "--batch" "-l" cand3))
                               (test-flow--set-conf sess 'runner 'external-command))
                             (list "emacs" "-Q" "--batch" "-l" cand3)))))))
